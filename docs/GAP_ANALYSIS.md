...
> 2026-09-23（WF-00A）：差距条目与证据保留；**优先顺序不再由本文的 H0/G1 依赖图决定**，改由 `docs/DEVELOPMENT_PLAN.md` 的 WF 交付包排序。特别是 H0.3（WorkspaceShell 拆分）不再是进入产品改造的前置条件，其剩余范围按 Task 04 修订段迁往 WF-00B/01/04；G1.4（MultiExec）→ WF-04C；G1.1 X11 spike 提前到 WF-01 期间；H0.5 Restore 契约随 WF-01、实现 → WF-07。
