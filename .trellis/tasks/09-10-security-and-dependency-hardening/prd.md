...
- 密钥检测覆盖当前受 Git 跟踪的工作树文件，以及完整 checkout 中所有可达 refs 的历史提交（含随后被删除的内容）；不扫描本机未跟踪的 `.env`、凭据或依赖缓存。
- 密钥命中阻断 CI。使用 Gitleaks CLI 的固定版本与校验过的官方二进制，不将扫描工具引入应用依赖。
- 依赖审计本阶段仅报告、归档和突出未解决项，不因 advisory 让 CI 失败，也不将“报告采集成功”视为安全验收或风险接受。工具不可用、网络失败、报告缺失/损坏仍须非零退出。
- 日志和 artifact 不含密钥、命中原文或完整源码片段；只输出定位与规则信息。测试夹具豁免须限定规则和具体证据，不允许整目录排除或自动接受所有历史命中。
- 不改变发布前逐项评估 Critical/High 的要求；CSP、GUI 回归和完整许可证门禁仍保留在后续验收中。
