...
- `ConnectionDialog` must treat `host_key_unknown` and `host_key_changed` from `connectionTestProfile(input)` as recoverable confirmation states, not ordinary errors. Parse the backend `details` payload through the shared host-key parser (`parseHostKeyError`, which matches the `kind`-tagged union; `raw_message` is no longer sent over IPC), render host, port, algorithm, and SHA256 fingerprints in the compact feedback card, and never show the raw payload to users.
